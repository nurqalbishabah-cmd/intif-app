...
DRIVE_FOLDER_ID = "https://drive.google.com/drive/folders/1eS8KFP9fp9XgfXO9S7zxznHjVh5BegSH?usp=drive_link"  # ID folder Google Drive tempat file diunggah
SCOPES = [
...
def get_drive_credentials():
    """Mendapatkan credentials Google Drive."""
    try:
        creds_dict = dict(st.secrets["gcp_service_account"])
...
def upload_to_drive(file_bytes: bytes, filename: str, mimetype: str, folder_id: str = None) -> str:
    """Upload file ke Google Drive dan kembalikan link yang bisa dibagikan."""
    ...
    try:
        ...
        # Buat file bisa diakses oleh siapapun yang punya link
    ...
    if st.button("Proses OCR & Lanjutkan →", key="btn_step1"):
        ...
        if errors:
            ...
            with st.spinner("Memproses OCR paspor... (5-15 detik)"):
                ...
                # Simpan bytes file untuk di-upload ke Drive saat submit
                foto_profil.seek(0)
                st.session_state.foto_profil_data = {
                    'bytes': foto_profil.read(),
                    'name': foto_profil.name,
                    'type': foto_profil.type or 'image/jpeg',
                }
                foto_paspor.seek(0)
                st.session_state.foto_paspor    = True
                st.session_state.foto_paspor_data = {
                    'bytes': foto_paspor.read(),
                    'name': foto_paspor.name,
                    'type': foto_paspor.type or 'image/jpeg',
                }
                if foto_amandemen:
                    foto_amandemen.seek(0)
                    st.session_state.foto_amandemen = True
                    st.session_state.foto_amandemen_data = {
                        'bytes': foto_amandemen.read(),
                        'name': foto_amandemen.name,
                        'type': foto_amandemen.type or 'image/jpeg',
                    }
                else:
                    st.session_state.foto_amandemen = False
                    st.session_state.foto_amandemen_data = None
                foto_kk.seek(0)
                st.session_state.foto_kk        = True
                st.session_state.foto_kk_data = {
                    'bytes': foto_kk.read(),
                    'name': foto_kk.name,
                    'type': foto_kk.type or 'image/jpeg',
                }
    ...
    if not st.session_state.submitted:
        ...
        if st.button("📤 KIRIM PERMOHONAN SEKARANG", key="btn_submit"):
            with st.spinner("Mengirim data ke sistem INTIF..."):
                ...
                # Upload file ke Google Drive dan dapatkan link
                st.spinner("Mengunggah dokumen ke Google Drive...")
                def _upload(key):
                    d = st.session_state.get(key)
                    if not d:
                        return ""
                    return upload_to_drive(d['bytes'], d['name'], d['type'], DRIVE_FOLDER_ID)

                data["FOTO PROFIL ID"]               = _upload('foto_profil_data')
                data["FOTO PASPOR HALAMAN DEPAN"]    = _upload('foto_paspor_data')
                data["FOTO PASPOR HALAMAN AMANDEMEN"]= _upload('foto_amandemen_data')
                data["SCAN KK/AKTA LAHIR"]           = _upload('foto_kk_data')
