...
def extract_passport_data(text: str) -> dict:
    ...
    # Kantor penerbit
    m = re.search(r'(?:Place of Issue|Tempat Pengeluaran|KANTOR)[:\s]+([A-Z][A-Z\s]+)', full, re.IGNORECASE)
    if m: result['kantor_penerbit'] = m.group(1).strip()[:30]

    # Kota lahir
    m = re.search(r'(?:Place of Birth|Tempat Lahir)[:\s]+([A-Z][A-Z\s]+)', full, re.IGNORECASE)
    if m: result['kota_lahir'] = m.group(1).strip()[:30]
